...
from search_agent.providers.factory import create_provider
...
def test_full_fake_run_succeeds():
	output = _run()
...
def test_expected_grouping_of_fake_leads():
	output = _run()
	by_name = {c.name: c for c in output.candidates}

	assert by_name["Rhône Plomberie"].classification.category == "high_fit"
...
def test_duplicate_same_domain_results_merge():
	...
	output = run_search_agent(
		agent_input, provider=DuplicatingProvider(), fetcher=FakePageFetcher()
	)
...
def test_failing_provider_produces_failed_or_warning_not_crash():
	...
	output = run_search_agent(
		example_input(), provider=ExplodingProvider(), fetcher=FakePageFetcher()
	)
...
def test_partial_failure_still_returns_candidates():
	...
	class FlakyProvider(SearchProvider):
		...
		def __init__(self):
			self._inner = FakeSearchProvider()
	...
	output = run_search_agent(
		example_input(), provider=FlakyProvider(), fetcher=FakePageFetcher()
	)
...
def test_create_provider_requires_api_key(monkeypatch):
	monkeypatch.delenv("TAVILY_API_KEY", raising=False)
	with pytest.raises(ProviderNotConfiguredError, match="TAVILY_API_KEY"):
		create_provider("tavily")


def test_create_provider_rejects_unknown_name():
	with pytest.raises(ProviderNotConfiguredError, match="Unknown search provider"):
