"""Example run of the Search Agent with the mock provider.

Usage (from backend/):
    poetry run python scripts/run_search_agent_example.py

Set SEARCH_PROVIDER=tavily|exa|brave|serper (with the matching API key in
the environment) to smoke-test a real provider instead of the mock.
...
def build_example_input() -> SearchAgentInput:
    return SearchAgentInput(
        ...
        provider_options=ProviderOptions(
            provider=os.environ.get("SEARCH_PROVIDER", "mock")  # type: ignore[arg-type]
        ),
    )
